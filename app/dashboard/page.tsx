...
import { getAppointments, getAutomationRules, getBudgets, getClinicSettings, getExpenses, getInventoryItems, getPatients, getPayments, getTreatments } from "@/lib/data";
...
export default async function DashboardPage() {
  const [settings, patients, appointments, treatments, budgets, payments, automationRules, inventoryItems, expenses] = await Promise.all([
    getClinicSettings(),
    getPatients(),
    getAppointments(),
    getTreatments(),
  ...
  const activeTreatments = treatments.filter((treatment) => ["approved", "in_progress", "budgeted"].includes(treatment.status));
  ...
  const activeAutomationRules = automationRules.filter((rule) => rule.is_enabled);
  const lowStockItems = inventoryItems.filter((item) => item.is_active !== false && Number(item.current_stock || 0) <= Number(item.minimum_stock || 0));
  const aiInsights = buildAiInsights({ patients, appointments, treatments, budgets, payments, inventoryItems });
  ...
  const scheduledPatientsWithoutFutureAppointment = patients.filter((patient) => patient.status === "scheduled" && !patientIdsWithFutureAppointment.has(patient.id));
  const operationalAlerts = await getGlobalOperationalAlerts("app");
  const requireAttentionCount = operationalAlerts.length;

  return (
    <AppShell operationalAlerts={operationalAlerts}>
      <div className="flex flex-col gap-2">
        <p className="text-sm font-semibold text-primary">Dental Pilot · {settings.country}</p>
        <h1 className="text-2xl font-bold text-ink sm:text-3xl">Dashboard operativo</h1>
        <p className="text-muted">Vista de trabajo para saber qué requiere atención hoy en la clínica.</p>
      </div>

      <section className="mt-6 grid gap-3 sm:grid-cols-3">
        <Link href="/appointments" className="rounded-3xl bg-primary px-5 py-4 text-center font-semibold text-white shadow-soft transition hover:bg-blue-700">
          <span className="inline-flex items-center justify-center gap-2"><PlusCircle size={18} /> Nueva cita</span>
        </Link>
        <Link href="/patients" className="rounded-3xl border border-slate-200 bg-white px-5 py-4 text-center font-semibold text-ink shadow-soft transition hover:border-primary/30 hover:text-primary">
          <span className="inline-flex items-center justify-center gap-2"><UserPlus size={18} /> Nuevo paciente</span>
        </Link>
        <Link href="/treatments" className="rounded-3xl border border-slate-200 bg-white px-5 py-4 text-center font-semibold text-ink shadow-soft transition hover:border-primary/30 hover:text-primary">
      ...
      <section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard title="Ingresos del mes" value={formatMoney(paidThisMonth, settings)} note="Pagos registrados" icon={<CreditCard size={22} />} />
        <MetricCard title="Egresos del mes" value={formatMoney(expensesThisMonth, settings)} note="Gastos operativos" icon={<CreditCard size={22} />} />
      ...
      {scheduledPatientsWithoutFutureAppointment.length > 0 ? (
        <section className="mt-6 rounded-3xl border border-amber-200 bg-amber-50 p-5">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            ...
            <Link href="/patients" className="btn-secondary whitespace-nowrap border-amber-200 bg-white text-amber-800 hover:bg-amber-100">
              Revisar pacientes
            </Link>
          </div>
        </section>
      ...
      <section className="mt-6 card p-5">
        <div className="flex items-center justify-between gap-3">
          ...
          <StatusPill label="Agenda real" variant="success" />
        </div>
