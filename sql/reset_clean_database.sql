-- Dental Pilot v2.0.5
-- Reinicio de base operativa: deja el sistema limpio con configuración y datos mínimos precargados.
...
begin
  ...
  if to_regclass('public.ai_logs') is not null then delete from public.ai_logs; end if;
  if to_regclass('public.conversations') is not null then delete from public.conversations; end if;
